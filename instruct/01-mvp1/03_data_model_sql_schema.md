# Data Model — SQL Schema (MVP1)

## Notes
- SQLite schema managed by migrations.
- Enforce referential integrity via foreign keys.
- Use `meta` table for schema versioning.
...
Indexes:
- idx_tx_month ON transactions(month)
- idx_tx_account_month ON transactions(account_id, month)
- idx_tx_category_month ON transactions(category_id, month)
...
Indexes:
- idx_ip_plan_due_month ON installment_payments(plan_id, due_month)
- idx_ip_due_month ON installment_payments(due_month)
- idx_ip_status ON installment_payments(status)
